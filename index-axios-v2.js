...
// const getFavouritesBtn = document.getElementById("getFavouritesButton");
...
async function retrieveBreedInfo(breedId, breeds) {
  ...
  favoriteButton.innerHTML = 'Click the image to favorite it'
...
const getFavouritesButton = document.getElementById("getFavouritesBtn")
getFavouritesButton.addEventListener('click', getFavorites)
...
async function getFavorites() {
  ...
  // console.log(favs)
  // for each carousel with breed images
  favs.forEach((favId, index) => {
    const img = document.createElement('img');
    img.src = `https://cdn2.thecatapi.com/images/${favId}.jpg`;
    img.classList.add('carousel-image');
    img.alt = 'breed image';
    img.style.display = index === 0 ? 'block' : 'none';  // show 1st image, hide others
